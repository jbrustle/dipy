...
from myParSignal import ParSignal
from myPerpSignal import PerpSignal
from myLegendreGaussianIntegral import LegendreGaussianIntegral
from myWatsonCoeff import WatsonSHCoefficients
from scipy.special import lpmv

import numpy as np
import scipy as sc

def Neumann_PGSE(x, grad_dirs, G, delta, smalldel, fibredir, roots):
    ...
    LePerp = np.zeros((Gshape_firstcol,1))
    ePerp=np.exp(LePerp)
    ePerp=np.transpose(ePerp)

    Lpmp=LePerp-LePar

    lgi = LegendreGaussianIntegral(Lpmp, 6)

    coeff = WatsonSHCoefficients(kappa)
    coeffMatrix=np.tile(coeff,(l_q,1))
    
    cosTheta = np.dot(grad_dirs,fibredir)
    badCosTheta = np.where((np.absolute(cosTheta)>1))
    cosTheta[badCosTheta] = cosTheta[badCosTheta]/np.absolute(cosTheta[badCosTheta])

    sh = np.zeros(np.shape(coeff))
    shMatrix = np.tile(sh,(l_q,1))
    


    for i in range(0,6,1):
        shMatrix[:,i] = np.sqrt((i + .25)/np.pi)
        ...
        tmp = np.zeros((np.size(cosTheta)))
        for j in range(0,(np.size(cosTheta)),1):
            tmp[j] = sc.special.lpmv(0,2*i,np.transpose(cosTheta[j]))


        tmp = np.transpose(tmp)
        shMatrix[:,i] = shMatrix[:,i]*tmp

    product=lgi*coeffMatrix*shMatrix
    E = product.sum(axis=1)
    E = 0.5*E*ePerp
    E = np.transpose(E)

    return E

def test_Neumann_PGSE():
    ...
    roots = 0
    x = np.array([0.5,0,1.2767])

    sample = Neumann_PGSE(x, grad_dirs, G, delta, smalldel, fibredir, roots)
    sample = sample[2:8]*1.E5

    error = abs(3.1562-abs(sample[0])) + abs(3.5335-abs(sample[1])) + abs(3.9796-abs(sample[2]))
    error = error + abs(2.2767-abs(sample[3])) + abs(4.2975-abs(sample[4])) + abs(3.2766-abs(sample[5]))
