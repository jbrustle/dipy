...
def LegendreGaussianIntegral(x,n):
    ...
    exact_shape = np.array(exact.shape)
    if (exact_shape[0]>1):
        exact = exact[0,:]
        exact = exact.reshape(1,exact_shape[1])
    ...
    tmp = np.zeros(np.size(I[:,0]))
    tmp2 = np.sqrt(np.pi)*sc.special.erf(sqrtx)/sqrtx
    tmp2 = np.squeeze(tmp2)


    for i in range(0,(exact.size),1):
        tmp[exact[0,i]]=tmp2[i]
...
def test_LegendreGaussianIntegral():
    sample = LegendreGaussianIntegral([0.5,0.78,0.36,0.48,0.74,0.95],6)
    sample = sample[1,:]
    error = abs(1.5821-abs(sample[0])) + abs(0.1513-abs(sample[1])) + abs(0.0110-abs(sample[2])) + abs(5.9087E-04-abs(sample[3]))
    error = error + abs(2.5123E-05-abs(sample[4])) + abs(8.7989E-07-abs(sample[5])) + abs(2.6112E-08-abs(sample[6]))
